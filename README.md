...
## 口コミとリンク
口コミの本文にリンクは入れない。投稿に含まれる URL は `post.php` が本文から外して `links` に退避し、
`tools/import_posts.py` が取り込み時にも落とす（承認制でも出口を開けない）。
出したいリンクは運営が `reviews.link_url` に 1 件だけ手で入れる。行き先は**その小屋の公式サイトか自社サイト**のみで、
それ以外は `build.py` の `REVIEW_LINK_HOSTS` に弾かれてビルドが落ちる。投稿由来の行には `rel="nofollow ugc noopener"` が付く。
