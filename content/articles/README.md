...
| `external` | | 他サイト（Emospot 等）の記事 URL。書くとページは作らず、小屋・ルートページからその URL へリンクするだけになる。本文は不要 |

## 体験記は Emospot、HutsGo はデータ派生

体験記（泊まった記録・写真）は Emospot の山カテゴリーに置き、HutsGo からは `external:` でリンクする。
同じ文章を両方に載せない（重複コンテンツで両方が損をする）。外部リンクのクリックは `article_click`
として計測する（小屋への送客率には混ぜない）。

HutsGo 側に本文を置くのは、**データから作れて、更新すれば正しくなり続けるもの**だけにする。
例: 予約受付開始日まとめ（`/booking/`）、テント場のある小屋、カードが使える小屋。
手書きの体験記でヤマハックと競っても勝てないが、年ごとに変わる事実の鮮度と正確さでは勝てる。
